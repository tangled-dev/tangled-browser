...
if [[ " ${NODEJS_DIST_UNIX[@]} " =~ " ${NODEJS_DIST} " ]]; then
    ...
    ../nodejs/node ../nodejs/npm install --scripts-prepend-node-path --build-from-source --target_arch=$1 --fallback-to-build
    ../nodejs/node ../nodejs/npm run dist --scripts-prepend-node-path
fi